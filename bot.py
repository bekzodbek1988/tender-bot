...
# ЖАМИ 13 ТА АЛОҲИДА ҲУЖЖАТ (НОМЛАРИ ВА ИНДЕКСЛАРИ ТЎЛИҚ АЖРАТИЛГАН)
DOCUMENTS = {
    # 1. 1k хати
    "1k_kafolat_xati": {
    ...
    # 12. 88k - Алоҳида Танишиб чиқдим хати
    "88k_tanishuv_xati": {
        "title": "Танишув хати",
...
async def generate_and_send(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text("⏳ Барча 13 та алоҳида ҳужжат (1k-12k, 77k, 88k, 99k) тайёрланмоқда...")
    ...
    await update.message.reply_text("✅ Жами 13 та файл (1k, 2k, 4k, 5k, 6k, 7k, 8k, 9k, 10k, 12k ва 77k, 88k, 99k) муваффақиятли юборилди!")
...
def main():
    ...
    print("Бот 13 та аниқ номланган ҳужжат режимида ишга тушди...")
    app.run_polling()
